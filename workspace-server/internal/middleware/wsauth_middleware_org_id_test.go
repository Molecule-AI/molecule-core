...
func TestWorkspaceAuth_ValidOrgToken_OrgIDNULL_DoesNotSetContext(t *testing.T) {
	...
	// orgtoken.Validate — org_id is NULL, so no org_id context key is set.
	mock.ExpectQuery(orgTokenValidateQuery).
		WithArgs(tokenHash[:]).
		WillReturnRows(sqlmock.NewRows([]string{"id", "prefix", "org_id"}).
...
func TestWorkspaceAuth_OrgToken_DoesNotPanicOnNilOrgID(t *testing.T) {
	// F1097: Validate() scans org_id as sql.NullString. When org_id is NULL
	// in the DB, .Valid is false and orgID stays "". The middleware must not
	// panic — org_id is simply not set on the Gin context.
	mockDB, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New: %v", err)
	}
