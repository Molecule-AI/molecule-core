...
// an org API token, so that orgCallerID(c) in org_tokens.go can read it and
// anchor newly minted tokens to the caller's org instead of NULL.
//
// Before the fix: c.Get("org_id") was always nil → orgCallerID(c) always
...
func TestAdminAuth_OrgToken_SetsOrgIDOnContext_F1097(t *testing.T) {
	...
	// WHERE id = $1 so that orgCallerID(c) can read it.
