...
// TestSeedInitialMemories_Truncation verifies that content > 100_000 bytes is
// silently truncated to the limit before INSERT so the DB constraint is not
// violated. This is the regression test for the #1066 compaction artefact.
...
type captureBroadcaster struct {
	events.Broadcaster // embed as interface so the override is called
