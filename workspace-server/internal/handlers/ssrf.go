...
// ssrf.go — shared SSRF defence (URL + IP range validation) and path
// traversal checks used across multiple handler files.
//
// validateRelPath is used by templates.go file operations.
// isSafeURL and isPrivateOrMetadataIP are used by a2a_proxy.go (A2A dispatch)
// and mcp_tools.go (MCP session routing).
