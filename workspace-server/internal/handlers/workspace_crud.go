...
// #1027: cascade-disable all schedules for the deleted workspaces so
	...
	if c.Query("purge") == "true" {
		...
		purgeStatements := []string{
			"DELETE FROM agent_memories WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM activity_logs WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_secrets WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_channels WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_config WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_memory WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_token_usage WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM approval_requests WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM audit_events WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workflow_checkpoints WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_artifacts WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM agents WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_auth_tokens WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM workspace_schedules WHERE workspace_id = ANY($1::uuid[])",
			"DELETE FROM canvas_layouts WHERE workspace_id = ANY($1::uuid[])",
		}
		for _, stmt := range purgeStatements {
			if _, err := db.DB.ExecContext(ctx, stmt, purgeIDs); err != nil {
				log.Printf("Purge error for %v: %v", allIDs, err)
			}
		}
