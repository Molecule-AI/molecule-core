...
func (h *TerminalHandler) HandleConnect(c *gin.Context) {
	...
	// #1609 security fix: if the caller claims a specific workspace identity
	// (X-Workspace-ID header), the bearer token — if present — must belong to
	// that claimed workspace. ValidateAnyToken accepted ANY valid org token,
	// allowing Workspace A to forge X-Workspace-ID: B and reach B's terminal
	// if A held any valid token. ValidateToken(ctx, callerID, tok) binds the
	// token to the claimed workspace identity. Legacy callers with no token
	// still fall through to the CanCommunicate hierarchy check.
