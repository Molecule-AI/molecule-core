...
Molecule AI's A2A routing works across any infrastructure configuration. When a workspace is registered, it publishes a reachable endpoint address to the platform registry. The platform's A2A proxy handles the cross-infrastructure handoff: the caller agent sends to the platform, the platform forwards to the target workspace's reachable endpoint. Neither the caller agent nor the target workspace needs a direct network path to the other.

This means you can run agents on [remote workspaces](/docs/blog/2026-04-20-remote-workspaces/) across AWS, GCP, Azure, and on-premises infrastructure — and have them communicate directly — without configuring a VPN between any of those environments. The platform handles cross-cloud routing. The tradeoff is additional latency through the control plane proxy — which is a deliberate design decision that keeps the infrastructure manageable for teams that don't want a global VPN mesh.
