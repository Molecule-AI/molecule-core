...
Platform Instructions is available on **Enterprise plans**. Org admins on Enterprise plans can configure global and workspace-scoped instructions via `PUT /cp/platform-instructions`. Standard plan orgs can view resolved instructions; only Enterprise plan orgs can create or modify them.
